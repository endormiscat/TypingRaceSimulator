...
public class RaceScreen extends JPanel
{
                      ...
                      boolean[] wristSupport, boolean[] energyDrink, boolean[] headphones)
    {
        ...
        if (nightShift)
            for (Typist t : typists) t.setAccuracy(t.getAccuracy() - 0.10);
    ...
    private JPanel buildFooter()
    {
        ...
        quit.addActionListener(e -> { timer.stop(); raceGUI.showConfig(); });
        panel.add(quit);
        return panel;
    }
    ...
    private void advanceTypist(int i)
    {
        ...
        if (caffeineMode)   acc += turnCount <= 10 ? 0.15 : -0.05;
        if (energyDrink[i]) acc += typists[i].getProgress() < passage.length() / 2 ? 0.10 : -0.10;
        acc = Math.min(1.0, Math.max(0.0, acc));

        if (Math.random() < acc) typists[i].typeCharacter();

        double mistypeChance = (1 - acc) * MISTYPE_BASE;
        if (headphones[i]) mistypeChance = Math.max(0, mistypeChance - 0.05);
        ...
        if (caffeineMode && turnCount > 10) burnoutChance += 0.05;
    ...
    {
        JPanel lanesPanel = (JPanel) getComponent(1);
        JPanel lane       = (JPanel) lanesPanel.getComponent(i);

        for (Component c : lane.getComponents())
        {
        ...
        if      (typists[i].isBurntOut())      statusLabels[i].setText("💤 Burnt out — " + typists[i].getBurnoutTurnsRemaining() + " turns");
        else if (typists[i].getJustMistyped()) statusLabels[i].setText("❌ Mistyped!");
        else if (hasFinished(i))               statusLabels[i].setText("✅ Finished!");
        else                                   statusLabels[i].setText(" ");
    }

    private boolean hasFinished(int i)
    {
        for (int j = 0; j < finishIndex; j++)
            if (finishOrder[j] == i) return true;
        return false;
    }
